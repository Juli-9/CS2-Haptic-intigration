...
= Zusammenfassung
Haptisches Feedback sorgt für eine deutlich bessere Immersion bei vielen Videospielen. Allerdings besitzen einige Spiele kein natives haptisches Feedback. Oft liegt es daran, dass keine Möglichkeit besteht dieses Feedback darzustellen. Beispielsweise bei Computerspielen, da üblicher Weise eine Maus und eine Tastatur keine Motoren oder ähnliches besitzen.
...
In dieser Arbeit präsentieren wir `HaptiCS`. Zwei Armbänder die mit Vibrationsmotoren ausgestattet sind und eine haptische Benutzerschnittstelle für Counter Strike 2 darstellen. Das Ziel dieser Benutzerschnittstelle ist eine erhöhte Immersion, sowie auch eine verbesserte Spielerfahrung.
...
= Beispielszenario
Ana möchte ihre Counter Strike 2 Elo erhöhen. Ihre Fähigkeiten würden das auch erlauben, allerdings hat sie große Probleme den Überblick über ihre restliche Munition in ihrem Magazin zu behalten. Sie merkt, dass, wenn sie sich zu sehr auf das Zielen und ihre Position fokussiert, ihr es schwer fällt auf die Anzeige zu schauen. Durch einen Freund wird sie Aufmerksam auf das HaptiCS Armband und kauft sich dieses um es auszuprobieren. Durch das zusätzliche Feedback wird sie deutlich aufmerksamer auf ihre Munitionsmenge, merkt allerdings auch, dass sich ihr Fokus auf das Zielen und ihre Position nicht verringert.

= HaptiCS
Dieses Kapitel enthält alle relevanten Informationen über HaptiCS, die haptischen Armbänder für Counter Strike 2.
Der vollständige Quellcode befindet sich im GitHub-Repository @haptics_github.
...
Der Microcontroller kommuniziert über eine serielle Schnittstelle mit dem Host-Computer. Auf diesem wird ein Python Skript ausgeführt, das in Verbindung mit einer `.cfg` Datei, die in den Counter Strike 2 Installationsordner abgelegt werden muss, die Notwendigen Daten sammelt und an den Microcontroller sendet. Die `.cfg` Datei aktiviert das Senden der notwendigen Spiel Informationen, die dann durch HTTP-Requests ausgelesen werden können. Die Informationen dieser HTTP-Requests werden nach erhalt in JSON-Dateien zusammengefast, in einer eigen entwickelten State Engine, umgesetzt als Python Skript, verarbeitet und danach an den Microcontroller gesendet. Der Microcontroller führt keine Berechnungen durch, er nutzt nur die in den JSON-Dateien enthaltene Daten um die Vibration der Motoren anzupassen.
...
Die JSON-Dateien setzen sich aus Daten aus einer Konfigurations Datei und den erhaltenen Daten aus den HTTP-Requests zusammen. Im Detail sind das:
...
Weiterhin wird auf dem Host-Computer über einen selbst entwickelten Daten Plotter die Vibrationsaktivität der Motoren dargestellt, sowie auch in der Konsole der Inhalt der JSON-Dateien ausgegeben.

Zum Zusammenfassen der Daten in JSON-Dateien nutzt das System ArduinoJson (Entwickelt von Benoit Blanchon). Weiterhin verwendet das System die folgenden Python Plug-ins:
...
Die `intensityPercent` ist zwar angepasst an die derzeit ausgerüstete Waffe, allerdings gibt es die Möglichkeit die Gesamtstärke innerhalb des `state_engine.py` Skripts anzupassen. Außerhalb davon gibt es keinen Weg für eine Nutzer*in die Gesamtstärke anzupassen.

(Zeile 123, ```python int(weapon["intensity"] * GESAMTSTÄRKE)```)

Die `firingPattern` ist aus einer Auswahl aus fünf verschiedenen Wertekurven für jede Waffe bestimmt.
...
Eine wichtige Erweiterung wäre eine direkt am Gerät einstellbare Gesamtintensität der Vibrationen gewesen. Derzeit kann diese lediglich durch eine Änderung im Python-Code angepasst werden. Für eine alltagstaugliche Nutzung wäre stattdessen ein Potentiometer oder eine vergleichbare Hardwarelösung sinnvoll, mit der die Intensität jederzeit individuell eingestellt werden könnte.
